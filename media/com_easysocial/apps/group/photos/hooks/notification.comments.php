    ...
    public function execute(&$item)
    {
        ...
        if ($item->context_type == 'albums.group.create') {
            ...
            if ($album->user_id == $item->target_id && $item->target_type == SOCIAL_TYPE_USER) {
                $item->title    = JText::sprintf('APP_GROUP_PHOTOS_NOTIFICATIONS_COMMENTED_ON_YOUR_PHOTO_ALBUM', $names);

                return;
            }

            // For other users, we just post a generic message
            $item->title    = JText::sprintf('APP_GROUP_PHOTOS_NOTIFICATIONS_COMMENTED_ON_USERS_PHOTO_ALBUM', $names, FD::user($album->user_id)->getName());
        ...
        if ($item->context_type == 'stream.group.upload') {
            ...
            if ($photo->user_id == $item->target_id && $item->target_type == SOCIAL_TYPE_USER) {
                $item->title    = JText::sprintf('APP_GROUP_PHOTOS_NOTIFICATIONS_COMMENTED_YOUR_PHOTO_SHARED_ON_THE_STREAM', $names);

                return;
            }

            // For other users, we just post a generic message
            $item->title    = JText::sprintf('APP_GROUP_PHOTOS_NOTIFICATIONS_COMMENTED_USERS_PHOTO_SHARED_ON_THE_STREAM', $names, FD::user($photo->user_id)->getName());
       ...
       if ($item->context_type == 'photos.group.updateCover') {
            ...
            // For other users, we just post a generic message
            $item->title    = JText::sprintf('APP_GROUP_PHOTOS_NOTIFICATIONS_COMMENTED_ON_USERS_PROFILE_COVER', $names, FD::user($photo->user_id)->getName());
       ...
       if ($item->context_type == 'photos.group.uploadAvatar') {
            ...
            // For other users, we just post a generic message
            $item->title    = JText::sprintf('APP_GROUP_PHOTOS_NOTIFICATIONS_COMMENTED_ON_USERS_PROFILE_PHOTO', $names, FD::user($photo->user_id)->getName());
        ...
        if ($item->context_type == 'photos.group.upload' || $item->context_type == 'photos.group.add') {
            ...
            // For other users, we just post a generic message
            $item->title    = JText::sprintf('APP_GROUP_PHOTOS_NOTIFICATIONS_COMMENTED_ON_USERS_PHOTO', $names, FD::user($photo->user_id)->getName());
