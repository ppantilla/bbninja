...
class SocialGroupAppPhotosHookNotificationLikes
{
    /**
    ...
    public function execute(&$item)
    {
        ...
        if ($item->context_type == 'albums.group.create') {
            ...
            if ($album->user_id == $item->target_id && $item->target_type == SOCIAL_TYPE_USER) {
                $langString     = FD::string()->computeNoun('APP_GROUP_PHOTOS_NOTIFICATIONS_LIKES_YOUR_ALBUMS', count($users));
            ...
            $langString     = FD::string()->computeNoun('APP_GROUP_PHOTOS_NOTIFICATIONS_LIKES_USERS_ALBUMS', count($users));
       ...
       if ($item->context_type == 'photos.group.updateCover') {
            ...
            if ($photo->user_id == $item->target_id && $item->target_type == SOCIAL_TYPE_USER) {
                $langString     = FD::string()->computeNoun('APP_GROUP_PHOTOS_NOTIFICATIONS_LIKES_YOUR_PROFILE_COVER', count($users));
                $item->title    = JText::sprintf($langString, $names);
            ...
            // For other users, we just post a generic message
            $langString     = FD::string()->computeNoun('APP_GROUP_PHOTOS_NOTIFICATIONS_LIKES_USERS_PROFILE_COVER', count($users));
        ...
        if ($item->context_type == 'photos.group.uploadAvatar') {
            ...
            // We need to determine if the user is the owner
            if ($photo->user_id == $item->target_id && $item->target_type == SOCIAL_TYPE_USER) {
                $langString     = FD::string()->computeNoun('APP_GROUP_PHOTOS_NOTIFICATIONS_LIKES_YOUR_GROUP_PICTURE', count($users));
                $item->title    = JText::sprintf($langString, $names);
        ...
        if ($item->context_type == 'stream.group.upload') {
            ...
            if ($photo->user_id == $item->target_id && $item->target_type == SOCIAL_TYPE_USER) {
                $langString     = FD::string()->computeNoun('APP_GROUP_PHOTOS_NOTIFICATIONS_LIKES_YOUR_PHOTO_SHARED_ON_THE_STREAM', count($users));
                $item->title    = JText::sprintf($langString, $names);
            ...
            // For other users, we just post a generic message
            $langString     = FD::string()->computeNoun('APP_GROUP_PHOTOS_NOTIFICATIONS_LIKES_USERS_PHOTO_SHARED_ON_THE_STREAM', count($users));
            $item->title    = JText::sprintf($langString, $names, FD::user($photo->user_id)->getName());
        ...
        if ($item->context_type == 'photos.group.upload' || $item->context_type == 'photos.group.add') {
            ...
            // We need to determine if the user is the owner
            if ($photo->user_id == $item->target_id && $item->target_type == SOCIAL_TYPE_USER) {
                $langString     = FD::string()->computeNoun('APP_GROUP_PHOTOS_NOTIFICATIONS_LIKES_YOUR_PHOTO', count($users));
                $item->title    = JText::sprintf($langString, $names);
            ...
            // For other users, we just post a generic message
            $langString     = FD::string()->computeNoun('APP_GROUP_PHOTOS_NOTIFICATIONS_LIKES_USERS_PHOTO', count($users));
            $item->title    = JText::sprintf($langString, $names, FD::user($photo->user_id)->getName());
