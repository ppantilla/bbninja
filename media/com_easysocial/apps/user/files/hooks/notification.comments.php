    ...
    public function execute(&$item)
    {
        ...
        if ($item->context_type == 'files.user.create') {
            ...
            if ($stream->actor_id == $item->target_id && $item->target_type == SOCIAL_TYPE_USER) {
                $langString     = FD::string()->computeNoun('APP_USER_FILES_NOTIFICATIONS_USER_POSTED_COMMENT_ON_YOUR_FILE', count($users));
            ...
            $langString     = FD::string()->computeNoun('APP_USER_FILES_NOTIFICATIONS_USER_POSTED_COMMENT_ON_USERS_FILE', count($users));
