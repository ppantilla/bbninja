...
class EasySocialControllerInstallation extends EasySocialSetupController
{
	...
	public function installApp( $appArchivePath , $target )
	{
		...
		// // @debug
		// // 23162 Debug purposes. Remove this when not debugging!
		// $result 			= new stdClass();
		// $result->state		= true;
	...
	public function installCopy()
	{
		...
		switch( $type )
		{
			case 'admin':
				...
				// Check if the admin folder exists before we copy it so that old files wouldn't be affected
				$tmp		= JPATH_ADMINISTRATOR . '/components/com_easysocial/themes/default.bak';
				...
				// If it doesn't exist, check if the theme folder already exists
				$defaultTheme 	= JPATH_ADMINISTRATOR . '/components/com_easysocial/themes/default';

				if (JFolder::exists($defaultTheme)) {

					// Backup the admin theme folder
					JFolder::copy($defaultTheme, $tmp);

					// Delete the admin theme folder when it's done
					JFolder::delete($defaultTheme);
				}
			...
			case 'site' :
				...
				// Check if the themes folder exists before we copy it so that old files wouldn't be affected
				$themes 	= array('wireframe', 'frosty', 'dark');

				foreach($themes as $theme) {
					$tmp		= JPATH_ROOT . '/components/com_easysocial/themes/' . $theme . '.bak';
					...
					// If it doesn't exist, check if the theme folder already exists
					$themeFolder 	= JPATH_ROOT . '/components/com_easysocial/themes/' . $theme;

					if (JFolder::exists($themeFolder)) {

						// Backup the theme folder
						JFolder::copy($themeFolder, $tmp);

						// Delete the theme folder after backing it up
						JFolder::delete($themeFolder);
					}
				}
			...
			case 'media':
				...
				// Check if the scripts folder exists before we copy it so that old files wouldn't be affected
				$tmp 		= JPATH_ROOT . '/media/com_easysocial/scripts.bak';
				...
				// If it doesn't exist, check if the theme folder already exists
				$scriptsFolder 	= JPATH_ROOT . '/media/com_easysocial/scripts';

				if (JFolder::exists($scriptsFolder)) {

					// Backup the scripts folder
					JFolder::copy($scriptsFolder, $tmp);

					// Delete the scripts folder after backing it up.
					JFolder::delete($scriptsFolder);
				}
